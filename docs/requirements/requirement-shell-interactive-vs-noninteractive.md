...
8. Walk `interactive` ids with a stdin redirect (`done <file` / here-doc) so `prompt_yes_no` hits EOF and auto-answers no (reject).  
9. Offer skip / quit / maybe on dest review, or chain three `(y/N)` questions. Mode SSOT does not license a four-way menu.
...
| AC-6 | When `TTY=1`, `sr_interactive` **MUST** leave fd 0 for `prompt_yes_no` (walk ids on another fd). Auto-answer from EOF on the id file is forbidden |
| AC-7 | Dest `interactive` asks **one** `prompt_yes_no` per unfenced file (yes=approve, no/Enter=reject). **MUST NOT** skip / quit / maybe (domain SSOT) |
