...
**Current state (2026-08-17):** Specialized **software-development** product. Historical origin **cli-template**. Domain SSOT is Active **2.15.0** (file-based JSON approval; dest `/etc/sudoers.d/{{service}}-{{user}}`; Type 1 `setup` / `interactive` live; pretty `commands[]` fidelity). Prevention catalog is Active **1.3.0**. Registry is populated — see `index.md`.
...
| Version SSOT | `1.6.2` (ship unit hard-assign) |
