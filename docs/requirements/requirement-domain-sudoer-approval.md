...
**Status**: Active (Version 2.4.0) — file-based JSON approval; Type 0 routed; Type 1 interactive/hook **law**; live LPU/dest **Gap**  
**Area**: domain  
...
Privilege types and F6 Cmnds are owned by `requirement-three-layer-privilege-model.md`. LPU identity is owned by `requirement-least-privilege-user.md`. Live Type 0 dispatcher catalog is owned by `requirement-shell-cli-interface.md` (lifecycle) **plus** the Type 0 domain verbs in §2.1.

**Routing honesty:** `help` / `about` **MUST NOT** list a verb that has no dispatcher `case` arm. Type 0 convert/submit/list/show/print-sudoers **are routed**. Type 1 names **are routed** and **fail closed** without euid 0. Live `useradd`, hook install, `sr_interactive` loop, and dest install to `/etc` are **Gaps** — do not claim domain complete.
...
A **TTY login** as `sudoer-adm` **MUST** be able to enter that approve path **only** via the login hook (`sudo -n` global binary `interactive`) or an explicit Type 1 `interactive` / `approve` / `reject`. Empty argv stays help. Running the CLI as euid `sudoer-adm` without F6 **MUST** fail closed (`authz`). Basename, schema, dest, hook, and review-loop rules stay in §2.2. Live dest install to `/etc`, live `useradd`, hook install, and the review-loop body remain a **Gap**.

#### Roles

| Role | Who | Type | May | Must not |
|------|-----|------|-----|----------|
| **Submitter** | Ordinary login (`id -un`), not the approver LPU | **0** | Convert; `add` / `update` / `remove-sudoer-request` **for themselves**; list/show **own** sidecar ids | Submit **for** another user; `approve` / `reject`; write `/etc/sudoers.d`; choose the dest basename |
...
| **Allocator** | This CLI’s Type 0 path | **0** | Allocate the request basename; write JSON into inbound `sudoer-approving/` | Trust a caller-supplied dest basename; skip self-scope |
...
4. Inbound is writable (default `/home/sudoer-adm/sudoer-approving`, or an explicit queue override). The CLI **allocates** the name.  
5. Input is request JSON, or sudoers text the CLI converts first.  
6. **Remove** is purpose-only JSON (no `commands`).

**Not a submit situation:** a grant for a colleague; wanting `/etc` written immediately; running `approve` as a normal user (Type 1 fail-closed).
...
| **Dest** | Approve add/update → `/etc/sudoers.d/{{service}}-{{user}}` from the **name**, never `*-remove` |
...
Every domain verb **MUST** have exactly one privilege type. Type 0 never writes `/etc`. Type 1 **MUST** run with euid 0 and `SUDO_USER==sudoer-adm` or a real root session.

| Verb | Type | Handler family | Operands / flags | Required behavior |
|------|------|----------------|------------------|-------------------|
| `sudoers-to-json` | 0 | `sr_sudoers_to_json` | stdin **xor** `--file PATH`; `--action add\|update`; `--purpose TEXT`; `--service NAME` optional; `--out PATH` optional | Convert a sudoers text fragment to request JSON. Infer service from Cmnds. Never queue. Never `/etc`. `visudo -cf` the input first. |
| `json-to-sudoers` | 0 | `sr_json_to_sudoers` | stdin **xor** `--file PATH`; `--out PATH` optional | Convert request JSON to sudoers text. `remove` → `# Purpose:` comments only. `visudo -cf` the output except comment-only remove. Never queue. Never `/etc`. |
| `print-sudoers` | 0 | `sr_print_sudoers` | stdout or draft path | Emit **Table A only** (F6 fragment). Never write `/etc`. |
...
| `--queue-root DIR` | `SUDOER_CLI_QUEUE_ROOT` | Set all three as `DIR/sudoer-approving`, `DIR/sudoer-approved`, `DIR/sudoer-rejected` |
...
**Worked dest:** `/etc/sudoers.d/folder-backup-leolio` (same order as project-sudoers-file `{{APP_NAME}}-{{TARGET_USER}}`)
...
2. `--queue-root` / `SUDOER_CLI_QUEUE_ROOT`  
3. Default under LPU home: `sudoer-approving`, `sudoer-approved`, `sudoer-rejected`

| Check | Rule |
|-------|------|
| Path | **Absolute**; last component **MUST NOT** be a symlink |
| Distinct | Three resolved realpaths **MUST** differ |
...
| Sequence | `n` walks **all three** resolved dirs |

Default modes (when `setup` creates them): requesting **3773** sticky+setgid; approved/rejected **0751**. Files after create: `0640`, group `sudoer-adm`. Submit: `umask 077` + exclusive create. Do **not** `mv` over an existing dest.
...
| Request | Live `/etc/sudoers.d` |
|---------|------------------------|
| `…-{{service}}-{{username}}-add-{{n}}.json` | install `{{service}}-{{username}}`; backup previous same dest |
| `…-{{service}}-{{username}}-update-{{n}}.json` | replace `{{service}}-{{username}}`; backup previous |
| `…-{{service}}-{{username}}-remove-{{n}}.json` | delete `{{service}}-{{username}}` only; **never** install `{{service}}-{{username}}-remove` |

Foreign files under `/etc/sudoers.d` (no product dest name) **MUST NOT** be deleted. Backup previous dest under `/etc/sudoers.bak/` before replace/remove.
...
| File | When to write |
|------|----------------|
| `/home/sudoer-adm/.bashrc` | **Always** (create if missing) |
| `/home/sudoer-adm/.profile` | **Only if** it exists **and** does **not** source `.bashrc`. Uncertain → **skip** |
...
**Gap:** live `setup` does not yet write this snippet. Do not claim the hook is installed.
...
**Gap:** the handler is a stub (`confirm_required` even on a TTY). Do not claim the review loop complete.

#### Warnings (not hard reject)

`sr_validate_body` / interactive **SHOULD** warn when a self-scoped Cmnd is a shell, `sudo`, `visudo`, or writes under `/etc/sudoers.d`. Approver still decides.
...
| **LPU home (default)** | `/home/sudoer-adm` |
| **Default queues** | `/home/sudoer-adm/sudoer-approving` (3773), `…/sudoer-approved`, `…/sudoer-rejected` |
...
| **Routed now** | Type 0 convert/submit/list/show/print-sudoers; Type 1 names fail closed without euid 0 |
| **Gap** | Live `useradd`/`userdel`; hook install; `sr_interactive` loop body; dest install to `/etc`; about LPU/F6/trust-tier fields |
| **Test isolation** | `SUDOER_CLI_ALLOW_TEST_ROOTS=1` plus `--queue-root` / per-dir flags. Fake `SUDOERS_D`. No live `useradd` unless `SUDOER_CLI_LIVE_LPU_TEST=1`. |
...
8. Claim Type 1 host mutation complete without live `useradd` / dest to `/etc`.  
...
| **TP-SR-PRIV-01** | `tests/test_domain_sr.sh` | have | Type 1 verbs: non-root → non-zero, no `/etc` write |
...
| **TP-SR-INT-02** | `tests/test_domain_sr.sh` | todo | `interactive` as Type 1 with `TTY=0` / `--json` → `confirm_required`, no hang |
| **TP-SR-INT-03** | `tests/test_domain_sr.sh` | todo | Hook snippet: skip when non-interactive / `SSH_ORIGINAL_COMMAND`; no `exit` |
| **TP-SR-INT-04** | `tests/test_domain_sr.sh` | todo | Empty inbound `interactive` (when loop live) exits 0 |
