...
**Status**: Active (Version 2.21.0) — dest fence table + independent incorrect-JSON Fence REQ  
**Area**: domain  
...
| Decide | If the JSON is broken, dest says so and does not ask yes/no. If it is valid, a person accepts or declines. | `sudo sudoer-cli interactive` |
...
**Human decides:** accept / decline / skip of a **valid inbound** request is a **person**. Helpful checks review the **file** (schema, visudo, hostile inode, User `ALL` / alias). Extra actor locks after elev, unpublished auto-reject heuristics, forcing A=B, and hiding inbound files are **blockage**, not an approval system.
...
Dest `approve` / `reject` / `interactive` **MUST** run this list **before** any yes/no. When a **Fence** matches: display the match in people/folder words; **MUST NOT** ask yes/no for that file. Each dest **Fence** row **MUST** point at an independent REQ.
...
Unknown keys anywhere → `invalid_json`. Basename **action** must match JSON `action` when present (`field_mismatch`). User/service SSOT is JSON when present.
...
6. For each pending id (basename sort): **fence first** (`requirement-incorrect-json-format`). If a fence **matches**: display the match in people/folder words; **MUST NOT** ask yes/no; continue to the next file. If **no** fence: show purpose + body (same contract as `show`); prompt **approve** / **reject** / **skip** / **quit**.  
